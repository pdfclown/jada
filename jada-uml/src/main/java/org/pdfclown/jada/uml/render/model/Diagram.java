...
public abstract class Diagram extends UmlNode {
  ...
  protected <T extends IndentPrintWriter> T writeCustomDirectives(List<String> customDirectives,
      T out) {
    customDirectives.forEach(out::println);
  ...
  private void writeCreditsComment(PrintWriter output) {
    output.println();
    output.println("""
        ' \
  ...
  private String writePlantumlSourceToFile() throws IOException {
    ...
    try (StringBufferWriter writer = createBufferingPlantumlFileWriter(pumlFile)) {
      writeTo(IndentPrintWriter.of(writer, null));
